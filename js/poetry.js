  ...
  shouldShowManagementButtons(poem) {
    ...
    if (typeof auth !== 'undefined') {
      ...
      if (auth.currentUser) {
        ...
        // 编辑员可以管理自己的作品
        if (auth.isEditor && auth.isEditor() && poem.author === auth.currentUser.username) {
          console.log('✅ Poetry: 编辑员管理自己的作品，显示管理按钮');
          return true;
        }
    ...
    if (typeof auth !== 'undefined') {
      ...
      if (auth.currentUser) {
        ...
        if (action === '编辑' && auth.isEditor && auth.isEditor()) {
          if (auth.currentUser.username === workAuthor) {
            console.log('✅ Poetry: 编辑员用户，可以编辑自己的作品');
            return true;
          } else {
            console.log('⚠️ Poetry: 编辑员不能编辑其他人的作品');
            return false;
          }
        }
  ...
  async verifyPassword(action, workAuthor = null) {
    ...
    if (typeof auth !== 'undefined') {
      ...
      if (auth.currentUser) {
        ...
        if (action === '编辑' && auth.isEditor && auth.isEditor()) {
          console.log('✅ Poetry: 编辑员用户，授权编辑操作');
